...
local default_config = {
  ...
  png_viewer = 'okular --unique',
  --  png_viewer = 'xdg-open --reuse-window', -- Linux
  keep_pdf = false,
  make_png = true,
  auto_open = true,
  continuous_compile = false,
  last_compiled = nil,
}
...
function M.toggle_continuous_compile()
  M.config.continuous_compile = not M.config.continuous_compile

  if M.config.continuous_compile then
    -- Create save autocmd
    M.save_autocmd = vim.api.nvim_create_autocmd('BufWritePost', {
      pattern = '*.m4',
      callback = function()
        -- Only compile if content changed since last compile
        local content = table.concat(vim.api.nvim_buf_get_lines(0, 0, -1, false))
        if content ~= M.config.last_compiled then
          M.compile_m4_to_png()
          M.config.last_compiled = content
        end
      end,
    })
    vim.notify('Continuous compilation ENABLED (F5 to disable)', vim.log.levels.INFO)
  else
    -- Remove autocmd if exists
    if M.save_autocmd then
      vim.api.nvim_del_autocmd(M.save_autocmd)
      M.save_autocmd = nil
    end
    vim.notify('Continuous compilation DISABLED', vim.log.levels.INFO)
  end
end
...
\end{document}
  ...
  if M.config.auto_open then
    local cmd = string.format('%s %s &', M.config.png_viewer, output_png)
    --vim.notify(string.format('Opening generated file : %s', cmd), vim.log.levels.INFO)
    vim.fn.system(cmd)
  end
end
